...
import com.piaoxue.weixins.mFFT.VisualizerView;
...
//----------------------------------------------------------------
	...
	mStatusTextView.setText("播放音乐中....");  
	   ...
	   {  
	       ...
	       root.addView(mVisualizerView);  
	       mFftView = new TextView(this);
	       mFftView.setText(fftStr);
	       root.addView(mFftView);
	         ...
	         //xOrdinate是x轴的总刻度，因为一次会传输过来1024个数据，每两个数据要画成一条直线，所以x轴我们分成1023段。你要是觉的太多了，也可以像我一样除以2，看自己需求了。
	          // int xOrdinate = (mBytes.length - 1)/2;
	           ...
	           {  
	               ...
	               format_byte = String.format("%03d", mBytes[i]);
	               fftStr += format_byte;
	               fftStr += " "; 
	           }
               //发送幅度值
   				if(isConnecting)
   				{
					if(flag_bulb3==2)
					{
	   						//float fuzhi = (float) (mBytes[i]/127.0*24);
	   						//int fuzhi_i= (int)fuzhi;
	   						//String send_fuzhi = String.format("F%02d\r\n", fuzhi_i);
							
	   						String send_fuzhi = String.format("GRB%03d%03d%03d\r\n", mBytes[0],mBytes[2],mBytes[3]);
		    				try 
		    				{				    	
		    			    	mPrintWriterClient.print(send_fuzhi);//发送给服务器
